...
class public_perscom_basecamp_addcombatentry extends ipsCommand
{
	public function doExecute( ipsRegistry $registry )
	{
		...
		$this->registry->output->addNavigation( $this->lang->words['base_camp'], 'app=perscom' );		
       	$this->registry->output->addNavigation( $this->lang->words['add_combat_record_entry'], NULL );
		...
		// Output HTML
		$this->registry->output->setTitle( $this->lang->words['add_combat_record_entry'] );
        $this->registry->output->addContent( $this->registry->output->getTemplate('perscom')->addCombatEntry( $this->personnel->loadPersonnel() ) );
       	$this->registry->output->sendOutput();
	}

	public function submitCombatRecord() {
		...
		$this->DB->insert( $this->settings['perscom_database_requests'], array( 
			...
			'date' => strtotime('now'),
			'description' => sprintf('Combat Record Update: %s', $this->request['combat_entry']),
			'type' => 'Combat Record Entry',
		...
		$this->DB->insert( $this->settings['perscom_database_records'], array( 
			...
			'entry' => $this->request['combat_entry'],
			'type' => 'Combat Record Entry',
			'award' => '',
			'rank' => '',
			'discharge_grade' => '',
